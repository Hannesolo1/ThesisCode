...
import util
from generate_features_ranking_Leiden import comp_t_total
...
def generate_features_ranking(dataset_name,
    ...
    for walk in walks:
        ...
        explainer = LRPExplainer(gnn_model)
        feature_importance_gnn = explainer.attribute(
            data_subgraph.x, data_subgraph.edge_index, data_subgraph.y
        )
        total_rel = feature_importance_gnn.sum()
        if total_rel > 1e-12:
            # avoid division by zero
            feature_importance_gnn = feature_importance_gnn / total_rel

        # Now add normalized relevances into the global accumulator
    ...
    # Write out ranking
    report_file = 'data/train_gnn_LRP_long/ranking_synthetic.csv'
    ...
    report_file = 'data/train_gnn_LRP_long/importance_synthetic.csv'
    ...
    report_file_time = f'time2/{base_name}_long.csv'
