...
def generate_features_ranking(dataset):
    ...
    while walk_idx < len(walks) and successful_subgraphs < iterations:
        ...
        Y = merge_small_communities(Y, H, min_community_size)
        community_counts_merged = Counter(Y)
        print(" - Merged community sizes:", dict(community_counts_merged))

        if len(community_counts_merged) < 2:
            print("[WARNING] Not enough classes after merging. Skipping this subgraph.")
            # Add a random walk if possible
            if extra_walk_count < max_extra_walks:
                new_walk = next(nx.generate_random_paths(G, 1, walk_length_local))
                walks.append(new_walk)
        ...
        print(" - Label counts after merging:", label_counts)

        if len(label_counts) < 2:
            print("[WARNING] Not enough classes after final check. Skipping this subgraph.")
            # Add a random walk if possible
            if extra_walk_count < max_extra_walks:
                new_walk = next(nx.generate_random_paths(G, 1, walk_length_local))
                walks.append(new_walk)
    ...
    if iter_counter > 0:
        ...
        report_file = 'data/train_Leiden_Mod/ranking_synthetic_testing.csv'
        ...
        report_file = 'data/train_Leiden_Mod/importance_synthetic_testing.csv'
