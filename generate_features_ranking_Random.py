...
import datetime
import matplotlib
matplotlib.use('Agg')  # use non-interactive backend so figures can be saved on a server
...
def sample_subgraph(G, sample_size=10000):
    ...
    # Filter out isolated nodes (nodes with no neighbors)
    connected_nodes = [node for node, degree in G.degree() if degree > 0]

    # Sample from the connected nodes
...
def generate_features_ranking(dataset_name, data_dir='./datasets', synthetic_dir='Synthetic/',node_threshold=100000, sample_size=50000):
    ...
    if 'Erdos' in dataset_name or 'Barabasi' in dataset_name:
        ...
        try:
            ...
            return  # Skip this dataset if there's an erro
        ...
        if nodes_count > node_threshold:
            ...
            return
            print(f"Graph is too large with {nodes_count} nodes. Sampling a subgraph...")
            G = sample_subgraph(G, sample_size=sample_size)
            nodes_count = nx.number_of_nodes(G)
            edges_count = nx.number_of_edges(G)
            print(f"Sampled subgraph with {nodes_count} nodes and {edges_count} edges.")
    ...
    iter_counter = 0
    time_subgraphs_total = datetime.timedelta(0)  # Accumulate per-subgraph feature computation times

    # generate random walks
    walks = nx.generate_random_paths(G, iterations, walk_length)
    walks = list(walks)

    # Handle potential NaN issues by checking for empty rows or NaN values in transition probabilities
    ...
    for walk in walks:
        ...
        try:
            ...
            continue  # Skip this subgraph or use a different strategy
        ...
        try:
            ...
            subgraph_image_dir = os.path.join("images/Random_Class_Assignment",)
    ...
    report_file = 'data/train_Random/ranking_synthetic.csv'
    ...
    report_file = 'data/train_Random/importance_synthetic.csv'
...
for dataset in datasets:
    # Check if "Erdos" or "Barabasi" is in the file name
    #if "Erdos" in dataset or "Barabasi" in dataset:
    generate_features_ranking(dataset)
# Calculate the sum of the list
# After all iterations, calculate the total time by summing all time deltas
